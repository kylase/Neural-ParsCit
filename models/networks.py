...
class WordCharLSTMCRF(nn.Module):
    ...
    def generate_mask(tensor: torch.Tensor, mask_value: int = 1) -> torch.Tensor:
        ...
        Args:
            tensor: torch.Tensor
            mask_value (int) the value which will be 0 after masking
    ...
    def forward(self, *inputs: List[torch.Tensor]) -> float:
        """
        Args:
        ...
        encoded = self._features(word_inputs, char_inputs)
